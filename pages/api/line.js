  ...
  function processMessage() {
    ...
    if (event.message.type !== "text") {
      ...
      if (admin[0].status == "SA" || admin[0].status == "SO") {
        ...
        let couponInCom = coupons.filter(
          (coupon) => coupon.companyRef === customer[0]._id
        );

        let couponUsed = groupByKey(couponInCom, "used");
          ...
          stream.on("end", function () {
            ...
            Jimp.read(buffer, function (err, image) {
              ...
              qr.callback = function (err, value) {
                if (value) {
                  // temp = value.result;
                  console.log('couponUsed[false] =====> ', couponUsed)
      ...
      if (event.message.text == "สอบถาม GroupID") {
        ...
        if (
          ...
          let couponUsed = coupons.filter(
            (coupon) =>
              coupon.companyRef === customer[0]._id && coupon.used === false
          );

          let group = groupByKey(couponUsed, "amount");
        ...
        if (admin[0].status == "SO" || admin[0].status == "SA") {
          replyCommand =
            "สอบถามยอด : สอบถามยอดคงเหลือคูปอง\nสอบถาม GroupID : เช็คเลข GroupID ของ LINE Group นี้ (คำสั่งนี้สามารถใช้ได้ครั้งแรกครั่งเดียว)";
        } else {
